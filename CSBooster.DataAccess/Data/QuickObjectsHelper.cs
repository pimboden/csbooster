...
namespace _4screen.CSB.DataAccess.Data
{
    internal static class DataObjectsHelper
    {
        ...
        public static SqlDataReader GetReaderAll<T>(Business.QuickParameters paras, int? useThisObjectType) where T : Business.DataObject, new()
        {
            ...
            {
                paras.CommunityID = paras.Communities.ToGuid();
            ...
            try
            {
                ...
                sql.Append("DECLARE @CommunityType   AS INT\r\n");
                sql.Append("DECLARE @CommunityStatus AS INT\r\n");
                ...
                {
                    sql.Append("SELECT @CommunityType = OBJ_Type, @CommunityStatus = OBJ_Status FROM hitbl_DataObject_OBJ WHERE OBJ_ID = @CTY_ID\r\n");
                    GetData.Parameters.Add(SqlHelper.AddParameter("@CTY_ID", SqlDbType.UniqueIdentifier, paras.CommunityID.Value));
                }
                else
                {
                    sql.Append("SET @CommunityType = 1\r\n");
                    sql.Append("SET @CommunityStatus = 2\r\n");
                }
                ...
                {
                    sqlWhere.Append("(\r\n");
                    sqlWhere.Append("  (@CommunityType = 1 AND @CommunityStatus = 2 AND hitbl_DataObject_OBJ.OBJ_Status = 2 OR (hitbl_DataObject_OBJ.OBJ_Status = 1 AND hitbl_DataObject_OBJ.USR_ID = @UserIDLogedIn)) OR\r\n");
                    sqlWhere.Append("  (@CommunityType = 1 AND @CommunityStatus = 1 AND hitbl_DataObject_OBJ.CTY_ID IN (SELECT DISTINCT hirel_Community_User_CUR.CTY_ID FROM hirel_Community_User_CUR WHERE hirel_Community_User_CUR.USR_ID = @UserIDLogedIn)) OR\r\n");
                    sqlWhere.Append("  (@CommunityType = 19 AND (hitbl_DataObject_OBJ.OBJ_Status = 2 OR (hitbl_DataObject_OBJ.USR_ID = @UserIDLogedIn)))\r\n");
                    sqlWhere.Append(") AND\r\n");
                    GetData.Parameters.Add(SqlHelper.AddParameter("@UserIDLogedIn", SqlDbType.UniqueIdentifier, paras.UserIDLogedIn));
                }
                ...
                if (!string.IsNullOrEmpty(paras.Country))
                {
                    ...
                    GetData.Parameters.Add(SqlHelper.AddParameter("@Country", SqlDbType.NVarChar, 5, paras.Country.PrepareLike(false, true)));
                }
                if (!string.IsNullOrEmpty(paras.Zip))
                {
                    sqlWhere.Append("(hitbl_DataObject_OBJ.OBJ_Zip LIKE @Zip) AND\r\n");
                    GetData.Parameters.Add(SqlHelper.AddParameter("@Zip", SqlDbType.NVarChar, 8, paras.Zip.PrepareLike(false, true)));
                }
                if (!string.IsNullOrEmpty(paras.City))
                {
                    sqlWhere.Append("(hitbl_DataObject_OBJ.OBJ_City LIKE @City) AND\r\n");
                    GetData.Parameters.Add(SqlHelper.AddParameter("@City", SqlDbType.NVarChar, 50, paras.City.PrepareLike(false, true)));
                }
                ...
                if (!string.IsNullOrEmpty(paras.Nickname) || !string.IsNullOrEmpty(paras.Title) || !string.IsNullOrEmpty(paras.Description))
                {
                    ...
                    if (!string.IsNullOrEmpty(paras.Nickname))
                    {
                        ...
                        GetData.Parameters.Add(SqlHelper.AddParameter("@Nickname", SqlDbType.NVarChar, 256, paras.Nickname.PrepareLike(true, true)));
                    }
                    if (!string.IsNullOrEmpty(paras.Title))
                    {
                        tempORStmt += "(hitbl_DataObject_OBJ.OBJ_Title LIKE @Title) OR\r\n";
                        GetData.Parameters.Add(SqlHelper.AddParameter("@Title", SqlDbType.NVarChar, 100, paras.Title.PrepareLike(true, true)));
                    }
                    if (!string.IsNullOrEmpty(paras.Description))
                    {
                        tempORStmt += "(hitbl_DataObject_OBJ.OBJ_Description LIKE @Description)\r\n";
                        GetData.Parameters.Add(SqlHelper.AddParameter("@Description", SqlDbType.NVarChar, paras.Description.PrepareLike(true, true)));
                    }
                ...
                if (!string.IsNullOrEmpty(paras.TitleLeftChar))
                {
                    ...
                    foreach (string item in list)
                    {
                        ...
                        GetData.Parameters.Add(SqlHelper.AddParameter(paraName, SqlDbType.NVarChar, 100, item.PrepareLike(false, true)));
                        if (strOR.Length == 0)
                            strOR = " OR ";
                        ic++;
                    }
                ...
                if (!string.IsNullOrEmpty(paras.GeneralSearch))
                {
                    if (paras.CatalogSearchType == DBCatalogSearchType.FreetextTable)
                    {
                        ...
                        GetData.Parameters.Add(SqlHelper.AddParameter("@ObjectGeneralSearch", SqlDbType.NVarChar, 100, paras.GeneralSearch));
                    }
                    else if (paras.CatalogSearchType == DBCatalogSearchType.ContainsTable)
                    {
                        ...
                        GetData.Parameters.Add(SqlHelper.AddParameter("@ObjectGeneralSearch", SqlDbType.NVarChar, 100, FullTextSearch.GetNormalForm(paras.GeneralSearch)));
                    }
                }
                ...
                switch (paras.SortBy)
                {
                    ...
                    case QuickSort.InsertedDate:
                        if (paras.ViewLogParams == null)
                            sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_InsertedDate {1}\r\n", relOrder, paras.Direction.ToString());
                        else
                            sql.AppendFormat("ORDER BY {0} VIW_InsertedDate {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.Viewed:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_ViewCount {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.Commented:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_CommentCount {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.RatedCount:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_RatedCount {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.RatedAverage:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_RatedAverage {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.IncentivePoints:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_IncentivePoints {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.MemberCount:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_MemberCount {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.Random:
                        sql.AppendFormat("ORDER BY NEWID() {0}\r\n", paras.Direction.ToString());
                        break;
                    case QuickSort.Agility:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_Agility {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.RatedConsolidated:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_RatedConsolidated {1}, hitbl_DataObject_OBJ.OBJ_StartDate Desc\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.FavoriteCount:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_FavoriteCount {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.Title:
                        sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_Title {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
                    case QuickSort.Accuracy:
                        if (!string.IsNullOrEmpty(paras.GeneralSearch))
                            sql.AppendFormat("ORDER BY {0} Rank DESC\r\n", relOrder);
                        else
                            sql.AppendFormat("ORDER BY {0} hitbl_DataObject_OBJ.OBJ_StartDate {1}\r\n", relOrder, paras.Direction.ToString());
                        break;
        ...
        public static SqlDataReader GetReaderAllFriends(Business.QuickParametersFriends paras)
        {
            ...
            try
            {
                ...
                if (!paras.FriendType.HasValue)
                    ...
                    GetData.Parameters.Add(SqlHelper.AddParameter("@UFR_TypeID", SqlDbType.Int, paras.FriendType.Value));
                ...
                if (!string.IsNullOrEmpty(paras.Nickname))
                {
                    GetData.Parameters.Add(SqlHelper.AddParameter("@USR_Nickname", SqlDbType.NVarChar, 258, paras.Nickname.PrepareLike(true, true)));
                }
                if (!string.IsNullOrEmpty(paras.Title))
                {
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_Title", SqlDbType.NVarChar, 258, paras.Title.PrepareLike(true, true)));
                }
                if (!string.IsNullOrEmpty(paras.Description))
                {
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_Description", SqlDbType.NVarChar, 258, paras.Description.PrepareLike(true, true)));
                }
        ...
        public static SqlDataReader GetReaderAllTags(Business.QuickParametersTag paras)
        {
            ...
            try
            {
                ...
                if (paras.Relevance != QuickTagCloudRelevance.NoRelevance)
                    sql.AppendLine("ORDER BY TGW_Relevance DESC");
                else
                    sql.AppendLine("ORDER BY OBJ_Title");
        ...
        public static string GetSelectString<T>(Business.DataObject listItem, Business.QuickParameters paras, SqlCommand cmd) where T : Business.DataObject, new()
        {
            ...
            /*sql.Append("hitbl_DataObject_OBJ.OBJ_EXT_ID,\r\n");
            sql.Append("hitbl_DataObject_OBJ.USR_ID,\r\n");
            sql.Append("hitbl_DataObject_OBJ.USR_Nickname,\r\n");
            sql.Append("hitbl_DataObject_OBJ.CTY_ID,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Type,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Title,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Description,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_TagList,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_InsertedDate,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_UpdatedDate,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_ViewCount,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_RatedAverage,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_SpezialXml,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_CommentCount,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_MemberCount,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_URLImageSmall,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_ShowState,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Featured,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Agility,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_StartDate,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_EndDate,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_City,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Status,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Parent_OBJ_ID,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_RoleRight,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Geo_Lat,\r\n");
            sql.Append("hitbl_DataObject_OBJ.OBJ_Geo_Long,\r\n");*/
        ...
        public static Guid? GetTagGuid(string tagWord)
        {
            ...
            try
            {
                ...
                if (returnValue != null)
                    tagId = returnValue.ToString().ToNullableGuid();
            }
            finally
            {
                connection.Close();

...
//  Updated:   #1.1.0.0    14.01.2008 / PT   Einbau Factor (RatedConsolidatedFactor)
...
SELECT @ObjectInserted");
                ...
                if (item.ObjectType == Helper.GetObjectType("Community").NumericId)
                {
                    string strTitle = string.Format("Profile_{0}", item.UserID).ToLower();
                    if (item.Title.ToLower() == strTitle)
                        GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_Type", SqlDbType.Int, Helper.GetObjectType("ProfileCommunity").NumericId));
                    else
            ...
            if (item.ObjectType == 0 || item.ObjectType == 5 || item.ObjectType == Helper.GetObjectType("User").NumericId || string.Format("Profile_{0}", item.UserID).ToLower() == item.Title.ToLower())
                return;
        ...
        {
            QuickCacheHandler.RemoveCache(item.ObjectID.Value.ToString());
...
WHERE	[OBJ_ID] = @OBJ_ID");
                ...
                if (item.ExternalObjectID.Length > 0)
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_EXT_ID", SqlDbType.NVarChar, 128, item.ExternalObjectID));
                else
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_EXT_ID", SqlDbType.NVarChar));
                ...
                if (item.descriptionLinked.Length > 0)
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_DescriptionLinked", SqlDbType.NVarChar, item.descriptionLinked));
                else
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_DescriptionLinked", SqlDbType.NVarChar));

                if (item.DescriptionMobile.Length > 0)
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_DescriptionMobile", SqlDbType.NVarChar, item.DescriptionMobile));
                else
                    GetData.Parameters.Add(SqlHelper.AddParameter("@OBJ_DescriptionMobile", SqlDbType.NVarChar));
        ...
        public static T Load<T>(UserDataContext udc, Guid? objectID, Guid? partnerID, string externalObjectID, ObjectShowState? showState, bool ignoreCache) where T : Business.DataObject, new()
        {
            ...
            try
            {
                ...
                if (objectID.HasValue)
                    sb.AppendFormat("hitbl_DataObject_OBJ.OBJ_ID = '{0}'\r\n", objectID);
                else if (!string.IsNullOrEmpty(externalObjectID) && partnerID.HasValue)
                    sb.AppendFormat("OBJ_EXT_ID = '{0}' AND PAR_ID = '{1}'\r\n", externalObjectID.Replace("'", "''"), partnerID);
                if (showState != null)
                    sb.AppendFormat("AND OBJ_ShowState = {0}\r\n", (int)showState.Value);
                GetData.CommandText = sb.ToString();
        ...
        private static void ManageTagListLog(Guid objectID, int objectType, TagWord tagWord, bool forDelete)
        {
            ...
            try
            {
                ...
                GetData.Parameters.Add(SqlHelper.AddParameter("@TGW_Wordlower", SqlDbType.NVarChar, tagWord.WordLower));

                if (!forDelete)
                    GetData.Parameters.Add(SqlHelper.AddParameter("@TGW_Word", SqlDbType.NVarChar, tagWord.Word));
                else
                    GetData.Parameters.Add(SqlHelper.AddParameter("@TGW_Word", SqlDbType.NVarChar));
        ...
        public ObjectAccessRight GetUserAccess(Business.DataObject dataObject)
        {
            ...
            try
            {
                ...
                if (dataObject.ObjectID.HasValue)
                    sqlCommand.Parameters.Add(SqlHelper.AddParameter("@ObjectId", SqlDbType.UniqueIdentifier, dataObject.ObjectID.Value));
                if (dataObject.CommunityID.HasValue)
                    sqlCommand.Parameters.Add(SqlHelper.AddParameter("@TargetCommunityId", SqlDbType.UniqueIdentifier, dataObject.CommunityID.Value));
                sqlCommand.Parameters.Add(SqlHelper.AddParameter("@CurrentUserId", SqlDbType.UniqueIdentifier, dataObject.userDataContext.UserID));
                sqlCommand.Parameters.Add(SqlHelper.AddParameter("@ObjectType", SqlDbType.Int, (int)dataObject.ObjectType));
                sqlCommand.Parameters.Add(SqlHelper.AddParameter("@Anonymous", SqlDbType.Bit, !dataObject.userDataContext.IsAuthenticated));

...
namespace _4screen.CSB.DataAccess.Data
{
    internal class DataObjectVideo
    {
        ...
        public static string GetInsertSQL(Business.DataObjectVideo item, SqlParameterCollection parameters)
        {
            ...
            return "INSERT INTO hiobj_Video ([OBJ_ID],[Width],[Height],[AspectRatio],[SizeByte],[External],[URLReferenced],[TypeReferenced],[Site],[OriginalLocation],[Location],[ConvertMessage]) VALUES (@OBJ_ID,@Width,@Height,@AspectRatio,@SizeByte,@External,@URLReferenced,@TypeReferenced,@Site,@OriginalLocation,@Location,@ConvertMessage)";
        }

        public static string GetUpdateSQL(Business.DataObjectVideo item, SqlParameterCollection parameters)
        {
            SetParameters(item, parameters);
            return "UPDATE hiobj_Video SET [Width] = @Width, [Height] = @Height, [AspectRatio] = @AspectRatio, [SizeByte] =  @SizeByte, [External] = @External, [URLReferenced] = @URLReferenced, [TypeReferenced] = @TypeReferenced, [Site] = @Site, [OriginalLocation] = @OriginalLocation, [Location] = @Location, [ConvertMessage] = @ConvertMessage";
        }

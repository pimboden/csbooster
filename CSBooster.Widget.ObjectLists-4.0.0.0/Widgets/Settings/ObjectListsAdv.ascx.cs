...
{
    public partial class ObjectListsAdv : System.Web.UI.UserControl, IWidgetSettings
    {
        ...
        private void FillControls(bool fillData)
        {
            ...
            if (fillData)
            {
                ...
                this.RcbDateConst.SelectedValue = XmlHelper.GetElementValue(xmlDocument.DocumentElement, "DateConstraint", "None");
            }
            ...
            foreach (RadComboBoxItem item in this.RcbDateConst.Items)
            {
                item.Text = languageShared.GetString("TextDateConstraint" + item.Value);
            }

            this.CbxDataSource.Items.Clear();
            if (ParentDataObject.ObjectType == Helper.GetObjectTypeNumericID("Page"))
            {
                CbxByUrl.Checked = XmlHelper.GetElementValue(xmlDocument.DocumentElement, "ByUrl", true);
        ...
        public bool Save()
        {
            try
            {
                ...
                XmlHelper.SetElementInnerText(xmlDocument.DocumentElement, "DateConstraint", this.RcbDateConst.SelectedValue);

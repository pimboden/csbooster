...
namespace _4screen.CSB.DataAccess.Business
{
    public class DataObject
    {
        ...
        internal FriendType friendVisibility = 0;
        ...
        public FriendType FriendVisibility
        {
            get { return friendVisibility; }
            set
            {
                if (friendVisibility != value)
                    objectState = ObjectState.Changed;
                friendVisibility = value;
            }
        }
        ...
        {
            if (objectState != ObjectState.Changed && GetXMLValue(propertyName) != value)
                objectState = ObjectState.Changed;
        ...
        private void ValiadateTagList()
        {
            string[] tagList = TagList.Split(Constants.TAG_DELIMITER);

            for (int i = 0; i < tagList.Length; i++)
            {
                if (tagList[i].Trim().Length > 100)
                {
                    throw new SiemeArgumentException("DataObject", "Validate", "TagList", "Tagword is longer than 100 characters");
                }
            }
        }
        ...
        public virtual void Insert(UserDataContext udc, Guid pObjectID, int parentObjectType)
        {
            Insert(udc, pObjectID, parentObjectType, null);
        }

        public virtual void Insert(UserDataContext udc, Guid pObjectID, int parentObjectType, string relationType)
        {
            Insert(udc);
            if (State == ObjectState.Saved)
            {
                Data.DataObject objData = new Data.DataObject();
                objData.RelInsert(new RelationParams { ParentObjectID = pObjectID, ParentObjectType = parentObjectType, ChildObjectID = ObjectID, ChildObjectType = ObjectType, Udc = udc, RelationType = relationType }, null);
            }
        }

        public virtual void Insert(UserDataContext udc)
        {

            if ((GetUserAccess(udc) & ObjectAccessRight.Insert) != ObjectAccessRight.Insert)
            {
            ...
            IncentivePointsManager.AddIncentivePointEvent(string.Format("{0}_UPLOAD", Helper.GetObjectType(ObjectType).Id.ToUpper()), udc, this.ObjectID.Value.ToString());
            if (ObjectType != Helper.GetObjectType("ForumTopicItem").NumericId)
                Event.ReportEvent(EventIdentifier.NewObject, udc.UserID, ObjectID);
        }

        /// <summary>
        /// Updates dataObject without access check -> never call from web ui
        /// </summary>
        public void UpdateBackground()
        {
            ...
            Data.DataObject.Update(this, UserDataContext.GetUserDataContext("admin"));
            if (statusChanged)
            {
        ...
        public virtual void Update(UserDataContext udc)
        {
            if ((GetUserAccess(udc) & ObjectAccessRight.Update) != ObjectAccessRight.Update)
            {
            ...
            Validate(AccessMode.Update);
            Data.DataObject.Update(this, udc);
            if (statusChanged)
            {
                Data.DataObject.UpdateCommunityChildObjects(this);
            }
            Event.ReportEvent(EventIdentifier.ChangeObject, udc.UserID, ObjectID);
            TrackingManager.TrackObjectEvent(ObjectType, ObjectID, TrackRule.Updated, string.Empty);
            //}
        }

        public void MarkAsDeleted(UserDataContext udc)
        {
            Delete(udc, true, false);
        }

        public void Delete()
        {
            Delete(UserDataContext.GetUserDataContext(), false, false);
        }

        public void Delete(UserDataContext udc)
        {
            Delete(udc, false, false);
        }

        public void Delete(UserDataContext udc, bool deep)
        {
            Delete(udc, false, deep);
        }

        public void Delete(UserDataContext udc, bool showStateOnly, bool deep)
        {
            if ((GetUserAccess(udc) & ObjectAccessRight.Delete) != ObjectAccessRight.Delete)
            ...
            if (deep)
                DeleteChildren(udc, ObjectID.Value);

            if (HttpContext.Current != null && HttpContext.Current.Items[ObjectID] != null)
                HttpContext.Current.Items.Remove(ObjectID);
            Data.DataObject objData = new Data.DataObject();
            objData.Delete(this, udc, showStateOnly);
        }

        private void DeleteChildren(UserDataContext udc, Guid parentId)
        {
            ...
            {
                DeleteChildren(udc, child.ObjectID.Value);
                child.Delete(udc);
            }
        }
        ...
        {
            return Load<T>(UserDataContext.GetUserDataContext(), objectID, null, null, ObjectShowState.Published, false);
        }
        ...
        {
            return Load<T>(UserDataContext.GetUserDataContext(), objectID, null, null, showState, ignoreCache);
        }
        ...
        public static DataObject LoadByReflection(Guid objectID, int objectType, ObjectShowState? showState)
        {
            string typeName = Helper.GetObjectType(objectType).Type;

            string cacheKey = "DataObject.Load" + typeName;
            if (showState.HasValue)
                cacheKey += showState.ToString();
            else
                cacheKey += "null";

            MethodInfo loadMethod = HttpRuntime.Cache[cacheKey] as MethodInfo;
            if (loadMethod == null)
            {
                Type type = null;
                ...
                loadMethod = typeof(DataObject).GetMethod("Load", new Type[] { typeof(Guid?), typeof(ObjectShowState?), typeof(Boolean)});
                loadMethod = loadMethod.MakeGenericMethod(type);
                HttpRuntime.Cache.Insert(cacheKey, loadMethod, null, System.Web.Caching.Cache.NoAbsoluteExpiration, new TimeSpan(0, 15, 0), System.Web.Caching.CacheItemPriority.AboveNormal, null);
            }
            return (DataObject)loadMethod.Invoke(null, new object[] { objectID, showState, false });
        }
        ...
        private static T Load<T>(UserDataContext udc, Guid? objectID, Guid? partnerID, string externalObjectID, ObjectShowState? showState, bool ignoreCache) where T : Business.DataObject, new()
        {
            ...
            if (objectID.HasValue)
            {
                if (HttpContext.Current != null)
                {
                    ...
                    if (!ignoreCache && HttpContext.Current.Items[cacheKey] != null)
                        ...
                        dataObject = LoadFormDB<T>(objectID, partnerID, externalObjectID, showState, ignoreCache);
                }
                else
                {
                    dataObject = Data.DataObject.Load<T>(objectID, partnerID, externalObjectID, showState, ignoreCache);
            ...
            if (dataObject.State != ObjectState.Added && (dataObject.GetUserAccess(udc) & ObjectAccessRight.Select) != ObjectAccessRight.Select)
            {
        ...
        private static T LoadFormDB<T>(Guid? objectID, Guid? partnerID, string externalObjectID, ObjectShowState? showState, bool ignoreCache) where T : Business.DataObject, new()
        {
            ...
            T item = Data.DataObject.Load<T>(objectID, partnerID, externalObjectID, showState, ignoreCache);
        ...
        public DataObject CopyToCommunity(UserDataContext udc, Guid? targetCommunityID, ObjectStatus enuStatus, string strTagList)
        {
            DataObject doNewObject = new DataObject(udc);
            ...
            doNewObject.UserID = udc.UserID;
            doNewObject.Nickname = udc.Nickname;
        ...
        // TODO: Check if this still works
        public DataObject CopyToCommunity(UserDataContext udc, Guid targetCommunityID)
        {
            DataObjectCommunity doTargetCommmunity = DataObject.Load<DataObjectCommunity>(targetCommunityID);
            return CopyToCommunity(udc, targetCommunityID, doTargetCommmunity.Status, doTargetCommmunity.TagList);
        }

        // TODO: Check if this still works
        public void DeleteWithCopies(UserDataContext udc)
        {
            if ((GetUserAccess(udc) & ObjectAccessRight.Delete) != ObjectAccessRight.Delete)
            {
            ...
            Data.DataObject objData = new Data.DataObject();
            objData.DeleteWithCopies(this, udc);
        }
        ...
        public void AddViewed(UserDataContext udc)
        {
            if (!ObjectID.HasValue)
            ...
            Data.DataObject objData = new Data.DataObject();
            objData.AddViewed(ObjectID.Value, ObjectType, udc);
        ...
        {
            if (!ObjectID.HasValue)
                throw new SiemeArgumentException("DataObject", "AddCommented", "ObjectId", "ObjectID is missing");

            if (ObjectType == 0)
                throw new SiemeArgumentException("DataObject", "AddCommented", "ObjectType", "ObjectType is missing");

            Data.DataObject objData = new Data.DataObject();
            objData.AddCommented(ObjectID.Value);
            TrackingManager.TrackObjectEvent(ObjectType, ObjectID, TrackRule.Commented, string.Empty);
        }

        public ObjectAccessRight GetUserAccess(UserDataContext udc)
        {
            ObjectAccessRight retval = ObjectAccessRight.None;
            if (udc.UserID != udc.AnonymousUserId)
            {
                if (udc.IsAdmin)
                {
                ...
                else
                {
                    ...
                    retval |= DataObject.GetUserAccess(udc, ObjectID, CommunityID, ObjectType);
                }
                return retval;
            }
            else
            {
                if (Helper.GetObjectTypeNumericID("Comment") != objectType)
                {
                    return ObjectAccessRight.None;
                }
                else
                {
                    return ObjectAccessRight.Insert | ObjectAccessRight.Select;
                }
            }
        }
        ...
        public static ObjectAccessRight GetUserAccess(UserDataContext udc, Guid? objectId, Guid? communityId, int objectType)
        {
            if (udc.IsAdmin)
            {
                return ObjectAccessRight.Insert | ObjectAccessRight.Update | ObjectAccessRight.Delete | ObjectAccessRight.Select;
            }
            else
            {
                return Data.DataObject.GetUserAccess(udc, objectId, communityId, objectType);
            }
        }

        public static void AddViewed(UserDataContext udc, Guid? objectID, int objectType)
        {
            if (!objectID.HasValue)
                throw new SiemeArgumentException("DataObject", "AddViewed", "ObjectID", "ObjectID is missing");
        ...
        public static bool AddRating(UserDataContext udc, Guid? objectID, int objectType, Business.Rating rating)
        {
            if (!objectID.HasValue)
                throw new SiemeArgumentException("DataObject", "AddRating", "ObjectID", "ObjectID is missing");

            Data.DataObject objData = new Data.DataObject();
            if (objData.AddRating(objectID.Value, objectType, udc.IsAuthenticated ? udc.UserID : udc.AnonymousUserId, udc.UserRole, rating))
            {
                Event.ReportEvent(EventIdentifier.Rate, udc.IsAuthenticated ? udc.UserID : udc.AnonymousUserId, objectID);
                TrackingManager.TrackObjectEvent(objectType, objectID, TrackRule.Rated, string.Empty);
                UserActivities.InsertRatedObject(udc, objectID.Value);
                QuickCacheHandler.RemoveCache("_" + objectID.Value);
            }
            // immer false damit nicht zweimal hintereinander gerated werden kann
            return false;
        }

        public static bool AddRating(UserDataContext udc, Guid? objectID, int objectType, int points)
        {
            Rating objRating = new Rating();
            objRating.SetAsStandard(1, points, objectID.Value);
            objRating.RatingObjects[0].VoteThis = points;

            return AddRating(udc, objectID.Value, objectType, objRating);
        }

        public static bool IsRatingPossible(UserDataContext udc, Guid? objectID)
        {
            Data.DataObject objData = new Data.DataObject();
            return objData.IsRatingPossible(objectID.Value, udc.IsAuthenticated ? udc.UserID : udc.AnonymousUserId, udc.UserRole, RatingType.Standard);
        }

        public static bool IsRatingPossible(UserDataContext udc, Guid? objectID, RatingType ratingType)
        {
            Data.DataObject objData = new Data.DataObject();
            return objData.IsRatingPossible(objectID.Value, udc.IsAuthenticated ? udc.UserID : udc.AnonymousUserId, udc.UserRole, ratingType);
        }
        ...
        public static void AddToFavorite(UserDataContext udc, Guid objectID, int objectType)
        {
            if (udc.IsAuthenticated)
            {
                Data.DataObject.ManageFavorite(objectID, objectType, udc.UserID, true);
            }
        }

        public static void RemoveFromFavorite(UserDataContext udc, Guid objectID)
        {
            if (udc.IsAuthenticated)
            {
                Data.DataObject.ManageFavorite(objectID, null, udc.UserID, false);
            }
        }
        ...
        public virtual string GetOutput(string outputType, string templatesUrl, XsltArgumentList argumentList)
        {
            return string.Empty;
        }
        ...
        public DataObjectList<T> GetChildren<T>(UserDataContext udc) where T : DataObject, new()
        {
            return GetChildren<T>(udc, false);
        }

        public DataObjectList<T> GetChildren<T>(UserDataContext udc, bool ignoreCache) where T : DataObject, new()
        {
        ...
        public DataObjectList<T> GetParents<T>(UserDataContext udc) where T : DataObject, new()
        {
            return GetParents<T>(udc, false);
        }

        public DataObjectList<T> GetParents<T>(UserDataContext udc, bool ignoreCache) where T : DataObject, new()
        {
        ...
        {
            if (!relParams.ParentObjectID.HasValue && !relParams.ChildObjectID.HasValue)
                new SiemeArgumentException("DataObject", "RelDelete", "ParentObjectID, ChildObjectID", "ParentObjectID and ChildObjectID can't both be null");
        ...
        public static void RelUpdate(UserDataContext udc, Guid? parentObjectId, Guid? childObjectId, int sortOrder)
        {
            Data.DataObject objData = new Data.DataObject();
            objData.RelUpdate(new RelationParams { Udc = udc, ParentObjectID = parentObjectId, ChildObjectID = childObjectId }, sortOrder);
        }

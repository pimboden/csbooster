    ...
    public class UserActivity
    {
        ...
        public override string ToString()
        {
            ...
            try
            {
                ...
                {
                    if (UserNickname == Constants.ANONYMOUS_USERNAME)
                        format = format.Replace("{U}", GuiLanguage.GetGuiLanguage("DataAccess").GetString("TextAnAnonymousUser"));
                    else
                        format = format.Replace("{U}", string.Format("<a href='{0}'>{1}</a>", Helper.GetDetailLink(Helper.GetObjectTypeNumericID("User"), UserNickname), UserNickname));
                }

                if (format.ToString().IndexOf("{T}") > -1)
                {
                    if (TargetObjectType == Helper.GetObjectTypeNumericID("ProfileCommunity"))
                        format = format.Replace("{T}", string.Format("<a href='{0}'>{1}</a>", Helper.GetDetailLink("User", TargetObjectText), TargetObjectText));
                    else
                        format = format.Replace("{T}", string.Format("<a href='{0}'>{1}</a>", Helper.GetDetailLink(TargetObjectType, TargetObjectID.ToString()), TargetObjectText));
                }
                ...
                if (format.ToString().IndexOf("{D}") > -1)
                    format = format.Replace("{D}", string.Format("<a href='{0}'>{1}</a>", Helper.GetDetailLink(DetailObjectType, DetailObjectID.ToString()), DetailObjectText));

...
namespace _4screen.CSB.DataAccess.Data
{
    internal class DataObjectUser
    {
        public static void FillObject(Business.DataObjectUser item, SqlDataReader sqlReader)
        {
            ...
            if (sqlReader["FacebookUserId"] != DBNull.Value) item.FacebookUserId = sqlReader["FacebookUserId"].ToString();
      ...
      [DisplayAdsShow] = @DisplayAdsShow";
        ...
        public static string GetFullTextWhereSQL(Business.QuickParameters qParas, SqlParameterCollection parameters)
        {
            if (!string.IsNullOrEmpty(qParas.GeneralSearch))
            {
                if (qParas.CatalogSearchType == DBCatalogSearchType.FreetextTable)
                    return string.Format(" OR FREETEXT(hiobj_User.*, '{0}', LANGUAGE 0x0)\r\n", qParas.GeneralSearch.Replace("'", "''"));
                else if (qParas.CatalogSearchType == DBCatalogSearchType.ContainsTable)
                    return " OR CONTAINS(hiobj_User.*, @ObjectGeneralSearch, LANGUAGE 0x0)\r\n";
                else
                    return string.Empty;
            }
            else
        ...
        private static void SetParameters(Business.DataObjectUser item, SqlParameterCollection parameters)
        {
            ...
            if (!string.IsNullOrEmpty(item.FacebookUserId)) parameters.AddWithValue("@FacebookUserId", item.FacebookUserId);
            else parameters.AddWithValue("@FacebookUserId", DBNull.Value);

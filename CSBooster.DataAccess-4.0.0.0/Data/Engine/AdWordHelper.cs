...
namespace _4screen.CSB.DataAccess.Data
{
    public class AdWordHelper
    {
        ...
        internal static string GetCampaignUrl(Guid campaignId, Guid? objectId, UserDataContext udc, string word, string type)
        {
            ...
            {
                if (type == "BannerLink")
                    IncreaseCreditsUsed(udc, campaignId, costPerBannerLinkClick);
                else if (type == "PopupLink")
                    IncreaseCreditsUsed(udc, campaignId, costPerPopupLinkClick);
                else
                    IncreaseCreditsUsed(udc, campaignId, costPerLinkClick);
        ...
        internal static void IncreaseCreditsUsed(UserDataContext udc, Guid campaignId, float creditsUsed)
        {
            SqlConnectionHelper sqlConnection = new SqlConnectionHelper();
            try
            {
                ...
                if (availableCredits < 0)
                {
                    ...
                    adWordFilterJob.ProcessDataObjectsInCampaign(udc, campaignId);
                }
            }
            finally
            {

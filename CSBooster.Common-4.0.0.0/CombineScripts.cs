...
namespace _4screen.CSB.Common
{
    public class CombineScripts
    {
        ...
        public static string CombineScriptBlocks(string scripts)
        {
            ...
            foreach (UrlMapSet mapSet in sets)
            {
                ...
                output = _FindScriptTags.Replace(output, new MatchEvaluator(delegate(Match match)
                                                                                {
                                                                                    ...
                                                                                    UrlMap urlMatch = mapSet.Urls.Find(new Predicate<UrlMap>(delegate(UrlMap map) { return map.Url == url; }));
                ...
                if (setStartPos >= 0)
                {
                    ...
                    string newScriptTag = "<script type=\"text/javascript\" src=\"/pages/other/Scripts.ashx?" + HttpUtility.UrlEncode(mapSet.Name) + "=" + HttpUtility.UrlEncode(setName) + "&" + HttpUtility.UrlEncode(urlPrefix) + "&" + HttpUtility.UrlEncode(SCRIPT_VERSION_NO) + "\"></script>";
        ...
        public static List<UrlMapSet> LoadSets()
        {
            ...
            using (XmlReader reader = new XmlTextReader(new StreamReader(HttpContext.Current.Server.MapPath("~/Configurations/CombineScriptsFileSets.config"))))
            {
